...
{
    public class LocalizationController : GameControllerMono
    {
        [System.Serializable]
        public class LocalizationControllerSettings : GameControllerSettings
        {
            [SerializeField] private LocalizationData _localizationData;

            public LocalizationData LocalizationData => _localizationData;

            public LocalizationControllerSettings() { }

            public void Create()
            {
                _localizationData = ScriptableObject.CreateInstance<LocalizationData>();
                _localizationData.Values = new List<LocalizationLanguage>();
            }
        }
        [SerializeField] private LocalizationControllerSettings _settings;

        protected Dictionary<SystemLanguage, LocalizationLanguage> Dictionary { get; private set; }

        public SystemLanguage Language { get; private set; } = SystemLanguage.English;

        protected bool IsInitialized;

        public LocalizationController SetSettings(LocalizationControllerSettings a)
        {
            _settings = a;
        ...
        protected override void OnInstall()
        {
            ...
            if (_settings.LocalizationData == null)
            {
                _settings.Create();
            }
            for (var index = 0; index < _settings.LocalizationData.Values.Count; index++)
            {
                var localization = _settings.LocalizationData.Values[index];
            ...
            if (PlayerPreferencesHelper.IsExist(PlayerPreferencesHelper.PlayerPreferencesType.Language))
            {
                try
                {
                    ...
                    var langData = _settings.LocalizationData.Values.Find(x => x.Language == lang);
                    if (langData != null)
                    {
        ...
        public SystemLanguage SetSystemLanguage()
        {
            ...
            var langData = _settings.LocalizationData.Values.Find(x => x.Language == systemLanguage);
            if (langData != null)
            {
        ...
        public LocalizationLanguage GetCurrent()
        {
            ...
            return _settings.LocalizationData.Values.Find(x => x.Language == Language);
        }
    }
}
