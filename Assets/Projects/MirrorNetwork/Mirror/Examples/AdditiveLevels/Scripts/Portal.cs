...
namespace Mirror.Examples.AdditiveLevels
{
    public class Portal : NetworkBehaviour
    {
        ...
        private void Update()
        {
           // labelText = Path.GetFileNameWithoutExtension(AdditiveLevelsNetworkManager.SceneLoad.name);
        ...
        IEnumerator SendPlayerToNewScene(GameObject player)
        {
            if (player.TryGetComponent<NetworkIdentity>(out NetworkIdentity identity))
            {
              ...
              //  SceneManager.MoveGameObjectToScene(player, AdditiveLevelsNetworkManager.SceneLoad);

                // Tell client to load the new subscene with custom handling (see NetworkManager::OnClientChangeScene).
            //    conn.Send(new SceneMessage { sceneName = AdditiveLevelsNetworkManager.SceneLoad.name, sceneOperation = SceneOperation.LoadAdditive, customHandling = true });
