...
namespace Mirror.Examples.AdditiveLevels
{
    ...
    public class AdditiveLevelsNetworkManager : NetworkManager
    {
        ...
        public void AddAdditiveScene(string a)
        {
            StartCoroutine(LoadSubScenes(a));
        }

        IEnumerator LoadSubScenes(string a)
        {
            yield return SceneManager.LoadSceneAsync(a, new LoadSceneParameters
            {
                loadSceneMode = LoadSceneMode.Additive,
                localPhysicsMode = LocalPhysicsMode.Physics3D // change this to .Physics2D for a 2D game
            });
            
      //      SceneLoad = SceneManager.GetSceneAt(SceneManager.sceneCount - 1);
