...
namespace MirrorNetwork
{
    ...
    public class NetworkAnimatorExtended : NetworkBehaviour
    {
        [Tooltip("Is AutoInitialize?")]
        ...
        bool CheckAnimStateChanged(out int stateHash, out float normalizedTime, int layerId)
        {
            ...
            if (animator == null || layerWeight == null) return false;

            float lw = animator.GetLayerWeight(layerId);

            if (layerWeight.Length <= 0) return false;
