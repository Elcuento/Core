...
{
    [CustomEditor(typeof(Rope3D))]
    public class Rope3DEditor : UnityEditor.Editor
    {
        public Rope3D Rope;
        ...
        void Input()
        {
            ...
            if (sceneView != null)
            {
                ...
                Plane plane = new Plane(sceneCamera.transform.forward, Rope.transform.position);
            ...
            if (guiEvent.type == EventType.MouseDown && guiEvent.button == 1 && guiEvent.shift)
            {
                ...
                {                    
                    ...
                    else
                    {
                            ...
                            Undo.RecordObject(Rope, "Add segment");
                            var maxPos = Rope.Segments[Rope.Segments.Count - 1].transform.position;
        ...
        void Draw()
        {
            ...
            for (int i = 0; i < Rope.Segments.Count; i++)
            {
                var f = Quaternion.identity; Vector2 newPos = Handles.FreeMoveHandle(Rope.Segments[i].transform.position, .1f,
